...
### Day 2 Q&A 기록
- **Q. Flask로 꼭 연습해야 해? (내 스택은 FastAPI인데)**
  → 아니. 라우팅/요청·응답 개념은 둘이 같아서 FastAPI로 연습해도 그대로 배워짐. Flask 고유한 건 Jinja2(서버가 HTML 생성) 정도. 커리큘럼의 과제 3을 Flask → FastAPI 재구축으로 교체.
- **Q. /docs는 자동 생성 페이지야? 의의가 뭐야?**
  → FastAPI가 내 코드(@app.get, 함수 등)를 읽고 자동으로 만드는 페이지. 비유: 서버=주방, /docs=메뉴판+시식코너. ①내 API가 뭘 할 수 있는지 보여주는 계약서(코드에서 뽑아내므로 항상 코드와 일치) ②Try it out으로 POST 등을 브라우저에서 테스트하는 도구.
- **Q. 엔드포인트가 뭐야?**
  → 서버의 주문 창구 하나. "이 주소로 오면 이 일을 해준다" = 주소 + 함수 한 쌍. `@app.get("/scores")` + `get_scores()` = 엔드포인트 1개. end+point = 요청이 도착하는 종착 지점.
- **Q. 재구축 연습 코드도 GitHub에 올려야 해?**
  → 올린다. ①백업 ②이 레포는 원래 연습장 ③커밋 히스토리가 "복붙 → 백지 재구축" 성장 궤적의 증거. 실수 포함 연습 코드 올리는 게 정상. 앞으로 세션 마무리 = 요약 + 푸시 세트.
