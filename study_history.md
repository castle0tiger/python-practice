...
### Day 14 — Docker 1일차: 첫 컨테이너 (18_docker/)
**한 것:** Docker Desktop 설치(PATH 미등록 → 임시 추가로 해결, VSCode 재시작 시 영구적용). 단순 FastAPI 서버를 Dockerfile로 이미지 빌드 → 컨테이너 실행 → 브라우저 접속 성공. docker exec로 상자 안에 들어가 python 3.11 확인(내 컴퓨터는 3.14 = 격리 증명). ps/rm/images로 생명주기 확인 (컨테이너 지워도 이미지는 남음).
**핵심 개념:**
- Dockerfile → 세이브를 어떻게 만들지 적은 설명서 (레시피)
- 이미지 → 그 설명서대로 찍어서 굳힌 세이브 파일 (읽기전용, 안 변함)
- 컨테이너 → 그 세이브를 불러와서 실제로 돌아가는 게임 (살아있음, 변함)
- 이미지:컨테이너 = 클래스:객체. "이미지"=CD이미지처럼 상태를 통째로 찍은 복사본. 불변이라 "어디서든 똑같이"가 보장됨.
