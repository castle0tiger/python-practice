...
[완료] 2단계: 입문 알고리즘 — 정렬 / 완전탐색 / 그리디 / 카운터·그룹핑 (Day 11~18)
        ↓
[완료] 3단계-1: 재귀·DFS (Day19~30) — 재귀5종+DFS5종+미니점검+마무리, 중첩구조 자유자재
        ↓
[현재] 3단계-2: A.트리(딕셔너리 DFS) → C.DP맛보기(피보나치+메모) → B.백트래킹(조합·순열)  ← 다음
```
...
## Day 27 — 중첩 리스트에서 값 찾기 ( ●●●●●○○○○○ 5/10 | Lv.2 | 코테 실전: 걸침 · DFS ) · "찾으면 멈춤" 탐색

**문제:** 값이 아무리 깊이 있어도 있으면 True. `nested_contains([1,[2,[3,4]],5], 4) → True`
...
def nested_contains(numbers, target):
    for n in numbers:
        if isinstance(n, list):
            if nested_contains(n, target):   # 파고든 재귀가 찾았으면(True)만
                return True                  # → True 반환. False면 넘어가 다음 형제
        else:
            if n == target:
                return True
    return False                             # for 다 돌아도 못 찾으면 (반복문 밖)
```

**핵심 함정 (스스로 의심→발견):** 처음 `return nested_contains(n, target)`(무조건 반환) → 첫 리스트 가지 못 찾으면 False를 그대로 반환·종료 → **뒤 형제 안 봄.** `[[1],[2,[3]]],3`에서 첫 `[1]`에 없자 False 종료 → 3 있는 `[2,[3]]` 못 봄. "다른 건 우연히 맞은 거 아니냐"를 스스로 의심(찾는 값이 첫 리스트에 있으면 우연히 맞음).
- 수정: **재귀가 True일 때만 return True, False면 계속.** = Day13 "존재 판정" 구조(`for: if 찾음 return True / return False`)에 재귀를 얹음.
- `return False`(마지막 줄)는 **호출마다 각자 실행**됨(Day26 "코드1개=실행여러개"). 안쪽 False가 바깥을 끝내는 게 아니라, 바깥은 그걸 받아 판단.

**스타일 — `== True` 생략:** `if 재귀() == True:` = `if 재귀():` (이미 T/F 반환하니 == True는 군더더기). 관례상 생략. `== False`는 `not`.
- 재귀 결과를 한 번만 쓰면 바로 if에(간결), 여러 번 쓰면 변수에 담기.

**→ 알고리즘 연결:** **DFS "탐색해서 찾으면 멈춤"(존재 판정).** Day24~26(끝까지 훑어 누적)과 달리 조기 종료 가능. Day13 존재판정 + DFS. 미로에서 "출구 있나?", 트리에서 "이 값 있나?"의 뿌리.

---

## Day 28 — 🔎 미니 점검: 중첩 리스트 짝수 세기 ( ●●●●●○○○○○ 5/10 | Lv.2 | 코테 실전: 걸침 · 재귀/DFS 종합 )

**문제(Day19~27 종합 점검):** 중첩 리스트의 짝수 개수. `count_evens([1,[2,[3,4]],6]) → 3`

**내 풀이 (5일 공백 후, 힌트·오류 없이 한 번에):**
```python
def count_evens(input):
    count = 0
    for i in input:
        if isinstance(i, list):
            count += count_evens(i)   # 리스트면: 파고든 개수를 받아 챙김 (Day24 핵심)
        else:
            if i % 2 == 0:            # 숫자면: 짝수일 때만
                count += 1
    return count
...
**점검 결과 — 무기 총동원 확인:** DFS 골격(isinstance+파고들기) + "재귀 결과 +=로 챙기기"(Day24, 그땐 6·0 버그로 배움→지금 자연스럽게) + 카운트 집계 + `%2==0` 짝수판별(Day2) + return 위치(Day13) + 지역변수 이해(Day24). **한 줄도 안 틀림.**

**Day24와의 관계:** `else: total += x`(무조건 더함) → `else: if x%2==0: count+=1`(짝수만). **DFS 골격 동일, "원소 처리"만 갈아끼움.** = DFS를 제대로 이해한 신호.

**종합:** 5일 공백 → 복습(Day27 정리) → 종합문제 한 번에 통과. 재귀/DFS(Day19~27) 체화 확인. 개별 개념이 아니라 "재귀적 사고"가 잡힘.
...
## Day 30 — 🏁 재귀/DFS 마무리: 중첩 리스트 종합 통계 ( ●●●●●●○○○○ 6/10 | Lv.2 | 코테 실전: 단골 · 재귀/DFS 마무리 )

**문제:** 중첩 리스트에서 개수·합·최대를 한 번에. `nested_stats([1,[2,[3,4]],5]) → {"개수":5,"합":15,"최대":5}`

**v1 (역할 분리 — 평탄화 후 각각 계산, 비효율 버전):**
```python
def nested_stats(input):
    result = {}
    result["개수"] = len(flatten(input))
    result["합"] = sum(flatten(input))
    result["최대"] = max(flatten(input))   # flatten을 3번 호출! (같은 구조를 3번 재순회)
    return result
```
**v2 (Day23 "결과 재사용" 적용 — 효율 개선, 자가 수정):**
```python
...
**v3 (재귀 하나가 셋을 동시에 — 어려움을 몸으로 확인):**
```python
def nested_stats_v3(input):
    result = {}
    count = 0
    total = 0
    biggest = None                          # 센티널: "아직 후보 없음"
    for i in input:
        if isinstance(i, list):
            sub = nested_stats_v3(i)         # 안쪽 통계(딕셔너리) 받기
            count += sub["개수"]
            total += sub["합"]
...
**여정 요약 — v1→v2→v3, 세 단계로 원칙을 몸에 새김:**
- v1: 정답이지만 `flatten` 3중 호출 = Day23 "재귀 결과를 변수에 담아 재사용"을 스스로 놓침 → 지적받고 즉시 v2로 수정.
- v3 시도: "재귀 하나가 다 하기"의 진짜 어려움을 직접 겪음 — Day29에서 배운 "한 함수가 다 하려 말고 나눠라"가 **왜 맞는 원칙인지 몸으로 증명.** 병합(sub의 세 값을 내 변수에 합치기)과 통계 추출(최댓값 판단)이 뒤섞이면 복잡도가 확 오름.
- 최종 버그: `sub`를 만들어놓고 최댓값 비교에서 안 씀(`nested_stats_v3(i)` 재호출 + `biggest = i`로 리스트 자체를 대입) — else 블록 패턴을 복붙하며 `i`를 `sub["최대"]`로 바꾸는 걸 놓친 것. 개념은 맞았고 실행 슬립이었음. 정확히 진단 후 스스로 고침.

**★ 센티널 값(sentinel value) — 오늘 처음 등장:**
- 지금까지 초기값 문제는 ①안전한 실제 첫 값이 있거나(`items[0]`, Day3·23) ②아예 다 모아서 마지막에 `max()`(Day26·29)로 피했음.
- 오늘은 **후보가 두 출처(숫자 branch / 리스트 branch)에서 실시간으로 섞여 나와서** 안전한 시작값이 없음 → 처음으로 `None`을 "아직 값 아님"이라는 **상태 표시**로 사용.
- `if biggest is None or 후보 > biggest:` — `or`의 **단락 평가**: 왼쪽이 참이면 오른쪽은 아예 안 봄. 그래서 `None > 숫자` 같은 비교 에러 없이, 첫 후보는 무조건 통과시키고 이후는 진짜 크기 비교.
- 대안(참고): `float('inf')`/`float('-inf')`를 센티널로 쓰는 방식도 실무에서 흔함(`is None` 체크 불필요해짐). 지금은 의도가 잘 보이는 `None` 방식이 학습엔 더 좋음.

**→ 알고리즘 연결:** **재귀/DFS 대장정(Day19~30) 마무리.** 재귀 5종 + DFS 5종 + 미니점검 + 두 문제(29,30)에서 "같은 결과, 다른 접근"을 실제로 비교. 오늘 나온 센티널 패턴은 다음 단계(A 트리, C DP)에서도 계속 재사용됨.
